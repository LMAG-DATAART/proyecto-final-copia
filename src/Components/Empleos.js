...
const Empleos = () => {
 ...
 { title: 'Desarrolladora Frontend', company: 'CodeWave',country:"United States", location: ' Austin TX', date: '2024-10-15', image: mujerFrontend },
  ...
  const handleSearch = async () => {
    const selectedCountry = document.getElementById('pais').value.toLowerCase();
          ...
          {filteredJobs.map((job, index) => (
            <div key={index} className={styles.jobCard}>
              ...
              <div className={styles.jobDetails}> 
                ...
                <button>Aplicar</button> {job.flag && <img src={job.flag} alt={`${job.country} flag`} className={styles.flagImage} />}
              </div>
            </div>
          ))}
        </div>
